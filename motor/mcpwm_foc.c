...
// Linear interpolation of the SynRM MTPA table from config: |I| (amps) -> id* (amps, <= 0).
// conf->foc_mtpa_lut[] is id*(|I|) in CENTIamps (0.01 A units, so |id*| up to ~328 A fits int16)
// on a uniform axis 0..foc_mtpa_lut_imax (MTPA_LUT_SIZE points), generated by
// docs/synrm/tools/mtpa_from_maps.py from the MotorXP FEA dq flux maps: for each |I| it is the
// d-current that maximises T=1.5p(psid*iq - psiq*id) on the current circle, capturing the heavy
// saturation/cross-coupling the constant-L closed form cannot. id* is always negative (reluctance
// harvesting) and |id*| < |I|. Uploadable from vesc_tool.
static float synrm_mtpa_id(const volatile mc_configuration *conf, float imag) {
	float imax = conf->foc_mtpa_lut_imax;
	if (imag <= 0.0f || imax <= 0.0f) {
		return 0.0f;
	}
	float fx = (imag / imax) * (float)(MTPA_LUT_SIZE - 1);
	if (fx >= (float)(MTPA_LUT_SIZE - 1)) {
		return (float)conf->foc_mtpa_lut[MTPA_LUT_SIZE - 1] * 0.01f; // hold past the last sample
	}
	int i0 = (int)fx;
	float t = fx - (float)i0;
	float a = (float)conf->foc_mtpa_lut[i0];
	float b = (float)conf->foc_mtpa_lut[i0 + 1];
	return (a + t * (b - a)) * 0.01f; // centiamps -> amps
}
