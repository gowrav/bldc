...
// conf->foc_mtpa_lut[] is id*(|I|) in AMPS (float32 — full +-450 A controller range, no overflow)
...
// harvesting) and |id*| < |I|. Above the table range the last sample is held. Uploadable from vesc_tool.
static float synrm_mtpa_id(const volatile mc_configuration *conf, float imag) {
	...
	if (fx >= (float)(MTPA_LUT_SIZE - 1)) {
		return conf->foc_mtpa_lut[MTPA_LUT_SIZE - 1]; // hold past the last sample
	}
	int i0 = (int)fx;
	float t = fx - (float)i0;
	float a = conf->foc_mtpa_lut[i0];
	float b = conf->foc_mtpa_lut[i0 + 1];
	return a + t * (b - a);
}
