...
// SynRM MTPA lookup table: id*(|I|) in milliamps, sampled on a uniform current-magnitude axis
// 0..SYNRM_MTPA_IMAX A (MTPA_LUT_SIZE points). Generated by docs/synrm/tools/mtpa_from_maps.py
// from the MotorXP FEA dq flux maps of the 9kW PMa-SynRM (p=2): for each |I| it is the d-current
// that maximises torque T=1.5p(psid*iq - psiq*id) on the current circle, capturing the heavy
// saturation/cross-coupling that the constant-L closed-form MTPA cannot. id* is always negative
// (reluctance harvesting) and |id*| < |I|. 3.2a bakes it in firmware; 3.2b makes it uploadable.
#define SYNRM_MTPA_IMAX			70.0f
static const int16_t m_synrm_mtpa_lut[MTPA_LUT_SIZE] = {
	0, -334, -1156, -2240, -3416, -4678, -6023, -7414, -8836, -10324, -11862,
	-13438, -15251, -16891, -18568, -20310, -21840, -23444, -24997, -26832,
	-29855, -32012, -34064, -36219, -38383, -40455, -42480, -44486, -46428,
	-48374, -50484, -53329, -55978
};

// Linear interpolation of the SynRM MTPA table: |I| (amps) -> id* (amps, <= 0).
static float synrm_mtpa_id(float imag) {
	if (imag <= 0.0f) {
		return 0.0f;
	}
	float fx = (imag / SYNRM_MTPA_IMAX) * (float)(MTPA_LUT_SIZE - 1);
	if (fx >= (float)(MTPA_LUT_SIZE - 1)) {
		return (float)m_synrm_mtpa_lut[MTPA_LUT_SIZE - 1] * 0.001f; // hold past the last sample
	}
	int i0 = (int)fx;
	float t = fx - (float)i0;
	float a = (float)m_synrm_mtpa_lut[i0];
	float b = (float)m_synrm_mtpa_lut[i0 + 1];
	return (a + t * (b - a)) * 0.001f; // milliamps -> amps
}
	...
	if (motor_now->m_state == MC_STATE_RUNNING) {
		...
		if (conf_now->foc_mtpa_mode != MTPA_MODE_OFF &&
			...
			if (conf_now->motor_type == MOTOR_TYPE_SYNRM) {
				...
				id_set_tmp = synrm_mtpa_id(fabsf(iq_ref));
			} else {
